...
## Linear Regression
Now that we have our combined dataset, we can now assess the relationship between a country's GDP per capita and the proportion of their population that has access to water. Before fitting a linear regression model, we would like to look at the relationship between the two variables over time.
...
One other notable observation that can be made from this animated plot is that the world region with seemingly the highest percentage of water access on average is Europe, while the region with the lowest percentage on average seems to be Africa.
...
summary(model)
```
Our estimated linear regression model (after performing a logit transformation on our response variable) is: log(ŷ/(100-ŷ)) = 1.516 + 0.0000463x. Since the intercept for our model is 1.516, then the logit value of having access to a basic water supply for countries with an average GDP per capita of zero is 1.516. Similarly, because our model's slope is 0.0000463, then when a country's GDP per capita increases by one dollar, the logit value of access to a basic water supply is expected to increase by 0.0000463.

### Model Fit

```{r}
kable(tibble(Access_Variance = var(log(gdp_v_water_mean$mean_access/(101 - gdp_v_water_mean$mean_access))),
