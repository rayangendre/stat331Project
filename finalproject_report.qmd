...
average_gdp <- gdp_v_water %>%
  group_by(year) %>%
  summarise(avg_gdp_pc = log(median(gdp_pc, na.rm = TRUE)))

average_water <- gdp_v_water |>
  group_by(year) %>%
  summarise(avg_water_access = log(median(access_pct, na.rm = TRUE)) )

average_water

ggplot() +
  geom_point(data = average_gdp, aes(x = year, y = avg_gdp_pc), color = "blue") +
  geom_point(data = average_water, aes(x = year, y = avg_water_access), color = "red") +
  labs(x = "Year", y = "Average Value", title = "Average Water Access Percentage vs Average GDP per Capita by Year") +
  theme_minimal()
...
gdp_v_water |>
  ggplot(aes(x = log(gdp_pc), y = log(access_pct) )) +
  geom_jitter() + 
