...
The final dataset contains 216 different countries over the years 2000 to 2022. Not every country has data for every year. For each country, only years that have data for both GDP and water access are included. (Describe data more)
...
gdp_v_water_mean |> 
  ...
  theme_bw() +
  labs(x = "GDP Per Capita (averaged over the years) (2017 US $)",
       y = "",
...
This plot also helps us assess the assumptions we make when doing a linear regression: Linearity, Homoscedasticity, Independence, and Normality. First looking at Linearity, the assumption that the relationship between GDP and Water Access is linear, as stated above it is clear that GDP Per Capita and the Logit of Water Access are not linearly related given the curve in the graph. Next, examining homoscedasticity, the assumption that all observations have the variance in the error, by examining the residual plot we have determined that due to a clear pattern existing in the residuals that this assumption is violated. Moving onto independence, the assumption that each observation is independent from another, is also mildly violated as we would expect geographically similar regions to have similar GDP per capitas. However, this should only mildly influence results. Finally looking at normality, the assumption that for any value of GDP, the water access is normally distributed, we have determined that this assumption is ok based on the QQ plot of the residuals.
...
The distribution of our R^2 values for our simulation ranges from 0.1 to 0.38, with a median approximately at 0.22. Our model struggles to describe the actual data which is seen in our low R^2 values. This struggle is expected as our data is not linear and has a curvature due to percentages being capped at 100% and GDP's being uncapped values.
